...
class FunnelChartConf extends Component {
    constructor(props) {
        super(props);
    ...
    loadDetailPageHandler() {
        ...
        $('#funnelFieldSelect', $(this.props.$parent)).html('<option>' + chartConfig.series[0].name + '</option>');
    ...
    loadUpdatePageHandler() {
        ...
        if (chartConfig.chartType === 'funnel') {
            ...
            $('input[name = "islegendDis"][value = "' + chartConfig.islegendDis + '"]', $(this.props.$parent)).attr('checked', 'checked');
            $('input[name = "isNumDis"][value = "' + chartConfig.isNumDis + '"]', $(this.props.$parent)).attr('checked', 'checked');
            $('#funnelFieldSelect', $(this.props.$parent)).select2().val(chartConfig.series[0].name).trigger('change');
    ...
    loadInitPageHandler() {
        ...
        $('#funnelFieldSelect', $(this.props.$parent)).select2().val('请选择').trigger('change');
    ...
    loadControlHandler() {
        if (this.props.operation === `reSearch`) {
            $('#funnelFieldSelect').html('<option>请选择</option>');
            $('#resultFieldSelect').html('<option>请选择</option>');
        }
        // 结果字段下拉控件配置项
        let funnelFieldObj = {};
        let funnelFieldArray = [];
        funnelFieldObj.id = 'funnelFieldSelect';
        ...
        for (let key in this.props.mqlResult.meta) {
            for (let col in this.props.mqlResult.meta[key]) {
                ...
                if (typeof (this.props.mqlResult.meta[key][col]) != 'object') {
                    funnelFieldArray.push(dataObj);
                }
            }
        }

        // 无可用字段提示
        if (this.props.mqlResult.meta && funnelFieldArray.length === 0) {
            warning('当前结果集格式不适配该图表');
        }

        funnelFieldObj.data = funnelFieldArray;
        // 结果字段下拉控件
        createSelect2(funnelFieldObj);
        bindSelectEvent(funnelFieldObj.id, 'select2:select', this.filterResultFieldHandler);
        createiCheck('input[ type = "radio"]', $(this.props.$parent));
    }
    ...
    filterResultFieldHandler(e) {
        ...
        let funnelField = $('#funnelFieldSelect').val().split('.')[0];
        let chartConfig = JSON.parse(this.props.selectedRecord.chartConfig);
        if (chartConfig.series) {
            funnelField = chartConfig.series[0].name.split('.')[0];
        }
        ...
        // 结果字段下拉控件数据项
        for (let col in this.props.mqlResult.meta[funnelField]) {
            let dataObj = {};
            dataObj.id = funnelField + '.' + col;
            dataObj.text = funnelField + '.' + col;

            // 结果字段为数值类型
            if (this.props.mqlResult.meta[funnelField][col] === 'NUMBER') {
    ...
    createChartHandler(chartOpt) {
        ...
        if ($('#funnelFieldSelect', $(this.props.$parent)).val() === '请选择') {
            warning('请选择标识字段');
            return;
        }
        ...
        localChartOpt.chartType = 'funnel';
        localChartOpt.title = $('#chartTitle', $(this.props.$parent)).val();
        localChartOpt.series = [];
        localChartOpt.series.push(this.seriesOptionsHandler($('#funnelFieldSelect').val(), $('#resultFieldSelect').val()));
     ...
     * @returns {{}}
     */
    seriesOptionsHandler(funnelField, resultField) {
        let seriesObj = {};
        let funnelCol = funnelField.split('.')[1];
        let resultCol = resultField.split('.')[1];
        seriesObj.name = funnelField;
        seriesObj.data = [];
        seriesObj.resultField = resultField;
        let mqlResult = this.props.mqlResult.result.mql[funnelField.split('.')[0]];
        for (let i = 0; i < mqlResult.length; i++) {
            let objArray = [];
            objArray.push(mqlResult[i][funnelCol]);
    ...
    render() {
        return (
            <div id="funnelChartConf">
                ...
                <div className="row">
                    <div name='funnelFieldSelect' className="form-group col-md-6">
                        <label htmlFor="funnelFieldSelect">标识字段</label>
                        <select id="funnelFieldSelect" className="form-control" style={{ width: '100%' }}>
                            <option>请选择</option>
                        </select>
                    </div>
...
export default FunnelChartConf;
