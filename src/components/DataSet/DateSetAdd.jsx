...
class DateSetCreate extends Component {
        ...
        $('#schema').on('click', 'tbody td .row-details', function() {
            ...
            if (row.child.isShown()) {
                for (var i = 0; i < $('input.fieldName').length; i++) {
                    if ($('input.fieldName').eq(i).val().length > 30) {
                        $('input.fieldName').eq(i).css('border', '1px #dd4b39 solid');
                        error('字段名最长为 30 个字符 !');
                        return;
                    } else {
                        $('input.fieldName').eq(i).css('border', '1px #d2d6de solid');
                    }
                    if (!/^(?!_)(?!.*?_$)/.test($('input.fieldName').eq(i).val())) {
                        $('input.fieldName').eq(i).css('border', '1px #dd4b39 solid');
                    ...
                    if (!/^(?!_)(?!.*?_$)[a-zA-Z0-9_]+$/.test($('input.fieldName').eq(i).val())) {
                        $('input.fieldName').eq(i).css('border', '1px #dd4b39 solid');
                ...
                $('button[name = "createNested"]', $('#nestRow_' + row.index())).off('click').on('click', function() {
                    ...
                    table.row.add({
                        'data': '',
                        'fieldName': '<input type="text" placeholder="支持英文、数字、下划线" maxlength= "30" class="fieldName form-control">',
    ...
    createDataSetHandler() {
        if ($('#datasetName', $('#datasetAdd')).val().trim() == '') {
            error('请输入数据集名 !')
            return;
        } else if (this.props.dataset.isExists) {
            error('数据集名已存在, 请重新输入 !')
            return;
        }
    ...
    handleCreateHandler() {
        ...
        table.row.add({
            'data': '',
            'fieldName': '<input type="text" placeholder="支持英文、数字、下划线" maxlength= "30" class="fieldName fieldNamer form-control">',
            'name': '<input type="text" class="name form-control">',
            'type': typeSelect,
            'store': storeSelect,
            'index': indexSelect,
    ...
    fieldInputHandler(e) {
        ...
        if (fieldName == '') {
            ...
            return;
        } else if (!/^(?!_)(?!.*?_$)/.test($(e.currentTarget).val())) {
            $(e.currentTarget).css('border', '1px #dd4b39 solid');
        } else if (type == 'date') {
            $(e.currentTarget).css('border', '1px #d2d6de solid');
            delete dateFieldObj[prevVal];
    ...
    partitionRuleHandler() {
        for (let i = 0; i < $('input[name="fieldName"]').length; i++) {
            let fieldName = $('input[name="fieldName"]').eq(i).val().trim();
            let type = $('input[name="fieldName"]').eq(i).closest('tr').find('select[name="type"]').val();
                    ...
                    <div className="box-body">
                        <div className="row">
                            <div className="datasetNameDiv col-md-6 form-group">
                                ...
                                <input type="text" id="datasetName" maxLength={30} name="datasetName" className="form-control" placeholder="30个字符以内，只允许含有小写英文、数字和下划线且下划线不能放首尾" onBlur={ this.isNameExistsHandler }/>
                            </div>
